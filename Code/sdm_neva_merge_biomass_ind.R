...
for(i in 1:nrow(dat.train)){
  ...
  # Prediction Maps
  source("~/GitHub/COCA/Code/sdm_predictionmaps_func.R")
...
for(i in seq_along(dir.cuts)){
  ...
  dat.dir.comp.tile<- data.frame("DIRECTIONAL.EFFECT" = c("Negative", "Neutral", "Positive"), "Binned" = c("Negative", "Neutral", "Positive"))
  
  dat.dir.comp.plot<- ggplot() +
    geom_text(data = dat.dir.comp.count, aes(x = DIRECTIONAL.EFFECT, y = Binned, label = Title), fontface = "bold") +
  ...
  dat.dir.comp.plot2<- dat.dir.comp.plot +
    geom_text_repel(data = dat.dir.labels, aes(x = DIRECTIONAL.EFFECT, y = Binned, label = toTitleCase(tolower(COMNAME)), color = Functional.Group), size = 3, segment.color = NA, nudge_y = -0.275, nudge_x = 0.005, show.legend = FALSE) +
